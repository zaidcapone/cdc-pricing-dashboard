        ...
        try:
            ...
            if article_columns:
                ...
                if st.button("🔍 Search All Articles (Dual Price Check)", type="primary", key=f"{client}_bulk_search"):
                        ...
                        for idx, article in enumerate(articles):
                            ...
                            if not prices_data.empty:
                                ...
                                if not price_matches.empty:
                                    ...
                                    # Get the most recent or relevant price
                    ...
                    # Detailed Analysis
                    with st.expander("📈 Detailed Price Analysis", expanded=False):
                        if not filtered_results.empty:
                            # Convert price columns to numeric for analysis
                            analysis_df = filtered_results.copy()
                            ...
                            analysis_df['Hist_Price_Num'] = analysis_df['Historical_Avg_Price'].apply(extract_price)
                            analysis_df['Curr_Price_Num'] = analysis_df['Current_Price'].apply(extract_price)
                            
                            # Only analyze rows with both prices
                            valid_prices = analysis_df.dropna(subset=['Hist_Price_Num', 'Curr_Price_Num'])
                            ...
                            if not valid_prices.empty:
                                ...
                                with col1:
                                    st.metric(
                                        ...
                                        f"${valid_prices['Hist_Price_Num'].mean() - valid_prices['Curr_Price_Num'].mean():.2f}",
                                        f"{(valid_prices['Curr_Price_Num'].mean() - valid_prices['Hist_Price_Num'].mean()) / valid_prices['Hist_Price_Num'].mean() * 100:.1f}%"
                                    )
                                
                                with col2:
                                    price_increased = len(valid_prices[valid_prices['Curr_Price_Num'] > valid_prices['Hist_Price_Num']])
                                    price_decreased = len(valid_prices[valid_prices['Curr_Price_Num'] < valid_prices['Hist_Price_Num']])
                                    st.metric("Price Trends", f"📈{price_increased} 📉{price_decreased}")
                            
                            # Show price distribution
                            st.subheader("📊 Price Distribution")
                            if not valid_prices.empty:
                                price_comparison = pd.DataFrame({
                                    'Historical Price': valid_prices['Hist_Price_Num'],
                                    'Current Price': valid_prices['Curr_Price_Num']
                                })
