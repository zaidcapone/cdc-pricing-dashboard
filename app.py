...
def logout_button():
    ...
    if st.button("🚪 Logout", key="logout_header"):
        ...
        st.rerun()
# ============================================
# NEW: DATASHEET MANAGER
# ============================================

def load_datasheet_mapping():
    """Load datasheet mapping from Google Sheets"""
    try:
        mapping_df = load_sheet_data("Datasheet_Mapping")
        if not mapping_df.empty:
            return mapping_df
    except:
        pass
    
    # Create empty mapping if sheet doesn't exist
    return pd.DataFrame(columns=['Article_Number', 'Product_Name', 'Supplier', 
                                 'File_Name', 'Folder_ID', 'Direct_Link', 'Status'])

def get_datasheet_info(article_number, supplier=None):
    """Get datasheet information for an article"""
    mapping_df = load_datasheet_mapping()
    
    if mapping_df.empty:
        return None
    
    # Try to find by article number
    matches = mapping_df[mapping_df['Article_Number'] == str(article_number)]
    
    if not matches.empty:
        return matches.iloc[0].to_dict()
    
    # If supplier provided, try to find in that supplier's folder
    if supplier:
        supplier_matches = mapping_df[
            (mapping_df['Article_Number'] == str(article_number)) & 
            (mapping_df['Supplier'] == supplier)
        ]
        if not supplier_matches.empty:
            return supplier_matches.iloc[0].to_dict()
    
    return None

def generate_google_drive_link(file_name, folder_id):
    """Generate Google Drive link from file name and folder ID"""
    if not file_name or not folder_id:
        return None
    
    # Google Drive search link
    return f"https://drive.google.com/drive/folders/{folder_id}?search={file_name}"

def display_datasheet_section(article_number, product_name, supplier=None):
    """Display datasheet section for a product"""
    
    datasheet_info = get_datasheet_info(article_number, supplier)
    
    if datasheet_info:
        file_name = datasheet_info.get('File_Name', '')
        folder_id = datasheet_info.get('Folder_ID', '')
        direct_link = datasheet_info.get('Direct_Link', '')
        status = datasheet_info.get('Status', '')
        ...
        if direct_link:
            ...
            <div style="background: linear-gradient(135deg, #E0F2FE, #BAE6FD); 
                        ...
                        margin: 1rem 0; border: 1px solid #0EA5E9;">
                <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 0.5rem;">
                    <span style="font-size: 1.5em;">📄</span>
                    <h4 style="margin:0; color: #0369A1;">Technical Datasheet Available</h4>
                </div>
                <p style="margin:0.5rem 0;"><strong>{product_name}</strong> ({article_number})</p>
                <div style="margin: 1rem 0;">
                    <a href="{direct_link}" target="_blank" 
                       style="color: #FFFFFF; background: linear-gradient(135deg, #0EA5E9, #0284C7); 
                              padding: 0.5rem 1.5rem; border-radius: 6px; text-decoration: none; 
                              display: inline-block; font-weight: bold;">
                       🔗 Open Datasheet (PDF)
                    </a>
                </div>
                <p style="margin:0; font-size: 0.9em; color: #475569;">
                    File: {file_name} | Status: {status}
                </p>
            </div>
            """, unsafe_allow_html=True)
            
            # PDF Preview (iframe)
            with st.expander("📊 Preview Datasheet", expanded=False):
                preview_url = direct_link.replace("/view", "/preview")
                st.markdown(f'<iframe src="{preview_url}" width="100%" height="500" style="border: 1px solid #ddd;"></iframe>', 
                          unsafe_allow_html=True)
                
        elif file_name and folder_id:
            # Search link (user needs to find file)
            search_url = generate_google_drive_link(file_name, folder_id)
            
            st.markdown(f"""
            <div style="background: linear-gradient(135deg, #FEF3C7, #FDE68A); 
                        padding: 1rem; border-radius: 8px; border-left: 4px solid #D97706; 
                        margin: 1rem 0; border: 1px solid #D97706;">
                <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 0.5rem;">
                    <span style="font-size: 1.5em;">📄</span>
                    <h4 style="margin:0; color: #92400E;">Datasheet Available (Manual Search)</h4>
                </div>
                <p style="margin:0.5rem 0;"><strong>{product_name}</strong> ({article_number})</p>
                <p style="margin:0.5rem 0; color: #92400E;">
                    ⚠️ Click below to search for the file in Google Drive
                </p>
                <div style="margin: 1rem 0;">
                    <a href="{search_url}" target="_blank" 
                       style="color: #FFFFFF; background: linear-gradient(135deg, #D97706, #B45309); 
                              padding: 0.5rem 1.5rem; border-radius: 6px; text-decoration: none; 
                              display: inline-block; font-weight: bold;">
                ...
                <p style="margin:0; font-size: 0.9em; color: #92400E;">
                    Search for: <strong>{file_name}</strong>
                </p>
            </div>
            """, unsafe_allow_html=True)
    else:
        ...
        <div style="background: #F3F4F6; padding: 1rem; border-radius: 8px; 
                    border-left: 4px solid #6B7280; margin: 1rem 0;">
            <div style="display: flex; align-items: center; gap: 10px;">
                <span style="font-size: 1.5em;">📄</span>
                <h4 style="margin:0; color: #6B7280;">Datasheet Status</h4>
            </div>
            <p style="margin:0.5rem 0;">No datasheet linked for <strong>{article_number}</strong></p>
            <p style="margin:0; font-size: 0.9em; color: #6B7280;">
                Contact administrator to link the datasheet
            </p>
        </div>
        """, unsafe_allow_html=True)
...
def display_product_card_flexible(product, available_columns):
    ...
    with st.expander(f"📦 {product['Article_Number']} - {product['Product_Name']}", expanded=False):
        ...
        # DATASHEET SECTION
        article_num = product['Article_Number']
        product_name = product['Product_Name']
        
        # Determine supplier
        supplier = None
        if 'Supplier' in available_columns and product['Supplier']:
            supplier = product['Supplier']
        
        # Display datasheet section
        display_datasheet_section(article_num, product_name, supplier)
        # ============================================
